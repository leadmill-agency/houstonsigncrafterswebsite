    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        message: [record.message, record.heardAbout ? `[How they heard about us: ${record.heardAbout}]` : ""]
          .filter(Boolean)
          .join("\n\n"),
        // Real attribution (2026-09-12): the visitor's own utm_* / gclid
        // tags and referrer, captured on arrival and carried through the
        // session. Previously utm_source was hardcoded to the domain,
        // which erased ad attribution (ChatGPT ads invisible in the CRM).
        landing_page: record.attribution?.landing_url || record.source,
        form_name: record.source,      // which form/page (kindMeta.source)
        utm_source: record.attribution?.utm_source
          || record.attribution?.referrer
          || "houstonsigncrafters.com",
        ...(record.attribution?.utm_medium && { utm_medium: record.attribution.utm_medium }),
        ...(record.attribution?.utm_campaign && { utm_campaign: record.attribution.utm_campaign }),
        ...(record.attribution?.utm_term && { utm_term: record.attribution.utm_term }),
        ...(record.attribution?.utm_content && { utm_content: record.attribution.utm_content }),
        ...(record.attribution?.gclid && { gclid: record.attribution.gclid }),
        ...(record.attribution?.gbraid && { gbraid: record.attribution.gbraid }),
        ...(record.attribution?.wbraid && { wbraid: record.attribution.wbraid }),
        ...(record.attribution?.referrer && { referrer: record.attribution.referrer }),
      }),
    });
    if (!res.ok) {
...
// --- OpenAI Ads Conversions API (server-side twin of the oaiq pixel) ---
// Sends lead_created to ads.openai.com with hashed identifiers so attribution
// and identifier coverage don't depend on the browser pixel alone. Deduped
// against the pixel by sharing record.id as the event id. Pure analytics:
// NEVER gates lead success, no-ops without the key, and times out fast.
// Docs: developers.openai.com/ads/conversions-api
const sha256 = (v) => createHash("sha256").update(v).digest("hex");

async function sendToOpenAICAPI(record) {
  const key = process.env.OPENAI_ADS_CAPI_KEY;
  if (!key) return false;
  const pid = process.env.OPENAI_ADS_CAPI_PID || "Kpb39q9fpSLgdgjoXfmgcW";
  try {
    const h = await headers();
    const jar = await cookies();
    const user = {
      emails_sha256: [sha256(record.email.trim().toLowerCase())],
    };
    const phoneDigits = (record.phone || "").replace(/\D/g, "");
    if (phoneDigits.length >= 8) user.phone_numbers_sha256 = [sha256(phoneDigits)];
    const ip = (h.get("x-forwarded-for") || "").split(",")[0].trim();
    if (ip) user.ip_address = ip;
    const ua = h.get("user-agent");
    if (ua) user.user_agent = ua;
    const obref = jar.get("__obref")?.value;
    ...
    const res = await fetch(`https://bzr.openai.com/v1/events?pid=${pid}`, {
      ...
      body: JSON.stringify({
        validate_only: false,
        integration_source: "hsc-nextjs-leads",
        events: [
          {
            id: record.id,
            type: "lead_created",
            timestamp_ms: Date.now(),
            source_url: h.get("referer") || BUSINESS.url,
            action_source: "web",
    ...
    if (!res.ok) console.warn(`[lead:${record.kind}] OpenAI CAPI ${res.status}: ${await res.text()}`);
    return res.ok;
  } catch (err) {
    console.warn(`[lead:${record.kind}] OpenAI CAPI failed:`, err?.message || err);
    return false;
  }
}
...
export async function submitLead(_prevState, formData) {
  ...
  // Ad attribution hidden fields (lib/attribution.js): the visitor's real
  // utm_* / gclid tags and external referrer, captured on arrival.
  const attribution = {};
  for (const key of [
    "utm_source", "utm_medium", "utm_campaign", "utm_term", "utm_content",
    "gclid", "gbraid", "wbraid", "referrer", "landing_url",
  ]) {
    const v = String(formData.get(key) || "").trim();
    if (v) attribution[key] = v.slice(0, 300);
  }
  const record = {
    id: randomUUID(),
  ...
  const [crmOk, kvOk, emailOk] = await Promise.all([
    ...
    // OpenAI Ads CAPI rides along; its result never affects lead success.
    sendToOpenAICAPI(record),
  ]);
  ...
  if (crmOk || kvOk || emailOk) {
    // leadId lets the client-side OpenAI pixel dedup against the CAPI event.
    return { ok: true, message: SUCCESS, leadId: record.id };
  }
